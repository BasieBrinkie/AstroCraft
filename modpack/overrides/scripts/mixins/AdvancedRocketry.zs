...
zenClass MixinRenderAsteroidSky {
    ...
    #   cancellable: true
    #}
    function beforeTessellatorDrawStar(buffer as BufferBuilder, sun as StellarBody, properties as DimensionProperties, solarOrbitalDistance as int, sunSize as float, sunColor as Vec3d, multiplier as float, ci as CallbackInfo) as void {
        ...
        var xrotangle as float = (((100.0f *(AdvancedRocketry.proxy.getWorldTimeUniversal(0) as float) % (48.0f * 24000.0f * orbitalPeriod)) / (48.0f * 24000.0f * orbitalPeriod)) * 360.0f);
        
        var zero as float = 0.0f;
        var one as float = 1.0f;

        // Make Star align with x-axis
        GL11.glRotatef(78, one, zero, zero);
    ...
    #   cancellable: true
    #}
    function afterTessellatorDrawStar(buffer as BufferBuilder, sun as StellarBody, properties as DimensionProperties, solarOrbitalDistance as int, sunSize as float, sunColor as Vec3d, multiplier as float, ci as CallbackInfo) as void {
        // Rotation angle, with rotation around star
        var orbitalDistance as float = properties.getSolarOrbitalDistance() as float;
        var solarSize as float = properties.getStar().getSize();
        var orbitalPeriod as float = (48.0f * pow(pow((orbitalDistance / (100.0f * solarSize)), 3.0f), 0.5f));
